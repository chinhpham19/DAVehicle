...
path = os.path.join(current_path, "Yolov8","coco.txt")
my_file = open(path, "r")
data = my_file.read()
class_names = data.split("\n")

# desired_classes = {class_names.index('Motorcycle'), class_names.index('Auto'), class_names.index('Car'), class_names.index('Bus'), class_names.index('Truck')}

desired_classes = {class_names.index('car'), class_names.index('motorcycle'), class_names.index('bus'), class_names.index('truck')}
...
#     return det_img
def draw_detections(image, boxes, scores, class_ids, track_id,mask_alpha=0.3):
    det_img = image.copy()
    ...
    # Lọc các phát hiện theo các lớp mong muốn
    filtered_boxes = []
    filtered_scores = []
    filtered_class_ids = []
    for class_id, box, score in zip(class_ids, boxes, scores):
    ...
    det_img = draw_masks(det_img, filtered_boxes, filtered_class_ids, mask_alpha)

    # Draw bounding boxes and labels of detections
    for class_id, box, score in zip(filtered_class_ids, filtered_boxes, filtered_scores):
        color = colors[class_id]
        draw_box(det_img, box, color)
...
def draw_box(image: np.ndarray, box: np.ndarray, color: tuple[int, int, int] = (0, 0, 255), thickness: int = 2) -> np.ndarray:
    x1, y1, x2, y2 = box.astype(int)
    line_length = 15  # Chiều dài của mỗi đoạn tại các góc
